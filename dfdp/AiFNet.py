...
class AiFDepthNet(nn.Module):
    ...
    def fit(self, x, args):
        ...
        out = self.out(Up_final)    # FIXME: bug here
        """
        result
        """
        B, C, S, H, W = out.shape
    ...
    def compute_loss(self, outputs, input_dict, args):
        ...
        if args['task'] == 'D_FS':
            ...
            losses[self.DISP_DEPTH] = l1(d_out[mask],gt_d[mask])
            losses['AiF'] = l1(AiF, gt_AiF)
            # # smoothness
            abs_fn = lambda x: x**2
            edge_constant = 150.
            img_gx, img_gy = self.image_grads(gt_AiF)
            ...
            losses['total'] = self.AiF_w * losses['AiF'] + self.disp_w * losses[
