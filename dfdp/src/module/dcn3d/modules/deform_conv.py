...
from dfdp.src.module.dcn3d.functions.deform_conv_func import DeformConvFunction
...
class DeformConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels,
        ...
        self.weight = nn.Parameter(torch.Tensor(
            out_channels, in_channels//groups, *self.kernel_size)).float()
        self.bias = nn.Parameter(torch.Tensor(out_channels)).float()
    ...
    def forward(self, input, offset):
        assert 3 * self.deformable_groups * self.kernel_size[0] * self.kernel_size[1] * self.kernel_size[2] == \
            offset.shape[1]
        return DeformConvFunction.apply(input.float(), offset.float(),
                                                   self.weight.float(),
                                                   self.bias.float(),
                                                   self.stride,
                                                   self.padding,
...
class DeformConvPack(DeformConv):
    ...
    def init_offset(self):
        self.conv_offset.weight.data.zero_().float()
        self.conv_offset.bias.data.zero_().float()

    def forward(self, input):
        offset = self.conv_offset(input.float())
        return DeformConvFunction.apply(input.float(), offset.float(),
                                          self.weight.float(), 
                                          self.bias.float(), 
                                          self.stride, 
...
class DeformConv_d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels,
                 kernel_size, stride, padding, maxsize=None, dimension='THW', dilation=1, groups=1, deformable_groups=1, im2col_step=64, bias=True):
    ...
    def forward(self, input, temp):
        ...
        return DeformConvFunction.apply(input, offset.float(),
                                        self.weight.float(),
                                        self.bias.float(),
                                        self.stride,
                                        self.padding,
...
class DeformConvPack_d(DeformConv_d):
    ...
    def __init__(self, in_channels, out_channels,
                 kernel_size, stride, padding, maxsize=None, dimension='THW',
                 dilation=1, groups=1, deformable_groups=1, im2col_step=64, bias=True, lr_mult=0.1):
        super(DeformConvPack_d, self).__init__(in_channels, out_channels,
                                             kernel_size, stride, padding, maxsize, dimension, dilation, groups, deformable_groups,
                                             im2col_step, bias)
        self.dimension = dimension
        self.length = len(dimension)
    ...
    def forward(self, input):
        temp = self.conv_offset(input.float())
        dimension_T = 'T' in self.dimension
        dimension_H = 'H' in self.dimension
        dimension_W = 'W' in self.dimension
        b, c, t, h, w = temp.shape
...
class DeformConvPack_dv2(DeformConv_d):

    def __init__(self, in_channels, out_channels,
                 kernel_size, stride, padding, maxsize=None, dimension='THW',
                 dilation=1, groups=1, deformable_groups=1, im2col_step=64, bias=True, lr_mult=0.1):
        super(DeformConvPack_dv2, self).__init__(in_channels, out_channels,
                                             kernel_size, stride, padding, maxsize, dimension, dilation, groups, deformable_groups,
                                             im2col_step, bias)
        
        # for offset range restriction
        self.maxsize = maxsize
    ...
    def forward(self, input):
        ...
        if self.length == 2:
            ...
            # offset range restriction (for stable learning)
            # if self.maxsize is not None:
            #     offset = torch.clamp(offset, min=-self.maxsize, max=self.maxsize)
        offset = offset.float()
