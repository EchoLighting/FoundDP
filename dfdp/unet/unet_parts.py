...
class ResDoubleConv(nn.Module):
    ...
    def forward(self, x):
        x = self.relu(self.conv2(self.relu(self.conv1(x))) + self.match_conv(x))
        return x
...
class OutConv(nn.Module):
    def __init__(self, in_channels, out_channels, active=None):
        super(OutConv, self).__init__()
        self.conv = nn.Sequential(
            ResDoubleConv(in_channels, in_channels),
            ResDoubleConv(in_channels, in_channels),
