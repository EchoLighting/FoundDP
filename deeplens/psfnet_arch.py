...
class MLP(nn.Module):
    ...
    def forward(self, inp):
        x = self.net(inp)
        psf = x.reshape(*x.shape[:-1],self.ks,self.ks)
        return psf
    
class MLP_lum(nn.Module):
    """ All-linear layer. This network suits for low-k intensity/amplitude PSF function prediction.
        choose l2loss for lum
    """
    def __init__(self, in_features, out_features, hidden_features=64, hidden_layers=3):
        super(MLP, self).__init__()
        self.ks = int((out_features-1)**(1/2))
        self.net = []
        self.net.append(nn.Linear(in_features, hidden_features//4, bias=True))
        self.net.append(nn.ReLU(inplace=True))
        self.net.append(nn.Linear(hidden_features//4, hidden_features, bias=True))
    ...
    def forward(self, inp):
        x = self.net(inp)
        psf = x[...,:-1].reshape(*x.shape[:-1],self.ks,self.ks)
        lum = x[...,-1:]
        return psf, lum
...
class MLPConv(nn.Module):
    ...
    def forward(self, x):
        ...
        output = decoded[:,0,:,:]
        b,ks,_ = output.shape
        output = output.view(*x.shape[:-1], ks, ks)
        return output
