...
class PSFNet(Lensgroup):
    def __init__(self, filename, model_name='mlp', kernel_size=11, sensor_res=(512, 512), device='cuda', same_sampling=False):
        super(PSFNet, self).__init__(filename=filename, sensor_res=sensor_res, device=device, same_sampling=same_sampling)
        self.device = device
    ...
    def load_net(self, net_path):
        ...
        pretrain_dict = torch.load(net_path,weights_only=True)
    ...
    def train_psfnet(self, iters=10000, bs=128, lr=1e-4, spp=2048, evaluate_every=10, result_dir='./results/temp', similarity=False):
        ...
        # Train the network
        # history of evaluation l1 losses for early stopping
        eval_l1_history = []
        early_stop_tol = 1e-5  # absolute tolerance for l1 loss closeness
        # We compare current eval l1 to the mean of the previous 5 evals
            ...
            with torch.no_grad():
                with torch.amp.autocast('cuda',dtype=torch.float16):
                    if (i+1) % evaluate_every == 0:
                        ...
                        # record evaluation L1 and check early stopping
                        try:
                            cur_l1 = float(l1_loss.item())
                        except Exception:
                            cur_l1 = l1_loss.item()
                        eval_l1_history.append(cur_l1)

                        # If we have at least 6 evaluations, compare current to mean of previous 5
                        if len(eval_l1_history) >= 6:
                            prev5 = eval_l1_history[-6:-1]
                            mean_prev5 = sum(prev5) / len(prev5)
                            if abs(cur_l1 - mean_prev5) < early_stop_tol:
                                logging.info(f"Early stopping: current l1 {cur_l1:.6e} close to mean(prev5) {mean_prev5:.6e}")
                                # save final model and return to stop training
    ...
    def evaluate_psf(self, result_dir='./'):
        ...
        test_foc_dists = torch.Tensor([-1000])
        test_dists = torch.Tensor([-600, -1100, -2100])
        ...
        for foc_z in test_foc_z:
            ...
            # depth = test_z * (self.d_max - self.d_min) + self.d_min
            ...
            # coc_pixel_radius = coc_pixel_radius.unsqueeze(0).unsqueeze(0).repeat(19,19,1)
            # x_gaussi_, y_gaussi_ = x_gaussi.unsqueeze(-1).repeat(1,1,3), y_gaussi.unsqueeze(-1).repeat(1,1,3)
            ...
            # l_mask,r_mask = torch.ones_like(psf_thin),torch.ones_like(psf_thin)
            # l_mask[:,0:9,:],r_mask[:,10:,:]= l_mask[:,0:9,:]*0,r_mask[:,10:,:]*0
            # psf_thin = psf_thin * r_mask
            # B,_,_=psf_thin.shape
            # values, indices = torch.max(psf_thin.view(-1,3),dim=0)
            # values = values.unsqueeze(0).unsqueeze(0)
            # psf_thin = psf_thin / (values+1e-6)
            # psf_thin = psf_thin.unsqueeze(0).repeat(3,1,1,1)
            # for i in range(3):
            #     self.vis_psf_map(psf_thin[...,i], filename=f'{result_dir}/foc{-foc_dist}_depth{-depth}_thin_{i}.png')
            # exit()
            for z in test_z:
                ...
                psf_gt = self.psf(points=points, ks=ks, center=True).reshape(3,ks,ks)
                ...
                inp = torch.stack((x, y, z_tensor), dim=-1).to(self.device)
                # print(psfnet(inp)[1])
                psf_pred = (psfnet(inp)).view(3,ks,ks)
                self.vis_psf_map(psf_pred, filename=f'{result_dir}/foc{-foc_dist}_depth{-depth}_pred.png')

                # COC需要改变，其二是需要考虑畸变，现在用主光线是没考虑畸变的（畸变不用考虑，因为chief可以被理解为已经做完畸变矫正）
                ...
                # psf_thin = psf_thin * psf_mask # Un-clipped Gaussian PSF
                # psf_thin_max = psf_thin.max().unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
                # psf_thin = psf_thin / psf_thin_max.repeat(3,1,1)
                ...
                coc_pixel = torch.clamp(coc / ps, min=0.1)
                coc_pixel_radius = coc_pixel / 2
                coc_pixel_radius = coc_pixel_radius.unsqueeze(-1).unsqueeze(-1)#.repeat(1,ks,ks)
                x_gaussi_, y_gaussi_ = x_gaussi.repeat(N,1,1), y_gaussi.repeat(N,1,1)
                psf_thin = torch.exp(- (x_gaussi_**2 + y_gaussi_**2) / (2 * coc_pixel_radius**2)) # We ignore constant term because PSF will be normalized later
                psf_mask = (x_gaussi_**2 + y_gaussi_**2 < coc_pixel_radius**2)
                psf_thin = psf_thin * psf_mask # Un-clipped Gaussian PSF
                l_mask,r_mask = torch.ones((ks,ks)).repeat(1,1,1),torch.ones((ks,ks)).repeat(1,1,1)
                ...
                psf = psf_l
                values, indices = torch.max(psf.view(N,-1),dim=-1)
                values = values.unsqueeze(-1).unsqueeze(-1)
                psf = psf / (values+1e-6)
                self.vis_psf_map(psf, filename=f'{result_dir}/foc{-foc_dist}_depth{-cur_depth}_thin_left.png')
    ...
    def pred_coc(self, inp, is_z=True):
        ...
        dtype = inp.dtype
        x_gaussi, y_gaussi = torch.meshgrid(
            torch.linspace(-ks/2+1/2, ks/2-1/2, ks, device=self.device, dtype=dtype),
            torch.linspace(-ks/2+1/2, ks/2-1/2, ks, device=self.device, dtype=dtype),
            indexing='xy'
        )
        ...
        z = inp[...,-1]
        B,W,H= inp.shape[0:3]
        foc_dist = torch.as_tensor(self.foc_d, device=self.device, dtype=dtype)
        ...
        psf_mask = (x_gaussi_**2 + y_gaussi_**2 < coc_pixel_radius**2)
        psf_thin = psf_thin * psf_mask # Un-clipped Gaussian PSF
        l_mask = torch.ones((ks, ks), device=self.device, dtype=dtype).repeat(1, 1, 1, 1, 1)
        r_mask = torch.ones((ks, ks), device=self.device, dtype=dtype).repeat(1, 1, 1, 1, 1)
        l_pixel, r_pixel = ks//2, ks//2+1
        near_focus_pos,far_focus_pos = torch.where(depth > foc_dist),  torch.where(depth < foc_dist)
        l_mask[...,0:l_pixel] = l_mask[...,0:l_pixel]*0
        r_mask[...,r_pixel:] = r_mask[...,r_pixel:]*0
    ...
    def psf_norm(self, psf, psf_occlu=None, centers_disp = None, eps=1e-6):
        norm_scale = self.simple_piecewise(centers_disp[:,2:3])
        B,W,H=psf.shape

        sums = psf.reshape(B, -1).sum(dim=-1).view(B, 1, 1)
        psf_norm = psf / (sums)
        psf_norm = psf_norm*norm_scale

        # chcek psf_norm max
        # psf_norm1= psf_norm.view(B, -1)
        # max1 = psf_norm1.max(dim=-1)

        if psf_occlu is not None:
            psf_occlu_norm = psf_occlu / (sums)
            psf_occlu_norm = psf_occlu_norm*norm_scale
            return psf_norm, psf_occlu_norm, norm_scale
        else:
            return psf_norm

    def simple_piecewise(self, x, A=600, B=40, C=1):
        mid = self.infocus_disp
        delta = (x-mid).abs() 
        fn = self.trainable_cam_param['fn']
        scale = 3*((delta*fn*(self.kernel_size//2))**2)+1 # 3.14*()+1
        scale = scale[:,:,None]
        return scale
    ...
    def compare_psf(self):
        ...
        test_dists = torch.Tensor([-500,  -1500, -20000])
        for depth in test_dists:
            ...
            self.vis_psf_map(psf, filename=f'./a_{depth}.png')
            psf = torch.stack([psfl[1], psfr[1]],dim=0)
            self.vis_psf_map(psf, filename=f'./b_{depth}.png')
            psf = torch.stack([psfl[1], psfr[1]],dim=0)
            self.vis_psf_map(psf, filename=f'./c_{depth}.png')

            # z = self.depth2z(depth)
            # z_tensor = torch.full_like(x, z)
            # inp = torch.stack((x, y, z_tensor), dim=-1)
            # inp = inp.repeat(1,1,1,1).to(self.device) # b w h 3 = 1,1,3,3
            # psf2 = self.pred(inp).detach().to("cpu")
            # # psf2 = self.pred_coc(inp).to("cpu")
            # # psf2 = self.pred_DPDNet(inp).to("cpu")
            # # psf2 = self.pred_Modeling(inp).to("cpu")
            # # psf2 = self.pred_Learn2reduce(inp).to("cpu")
            # psfstack = torch.stack([psf2[0,0,0,0,...],psf2[0,0,0,1,...]],dim=0)
            # self.vis_psf_map(psfstack, filename=f'./a_{z}.png')
            # psfstack = torch.stack([psf2[0,0,1,0,...],psf2[0,0,1,1,...]],dim=0)
            # self.vis_psf_map(psfstack, filename=f'./b_{z}.png')
            # psfstack = torch.stack([psf2[0,0,2,0,...],psf2[0,0,2,1,...]],dim=0)
            # self.vis_psf_map(psfstack, filename=f'./c_{z}.png')
    ...
    def render(self, img, depth, foc_dist, train=False):
        ...
        """
        # Use fp16 for rendering on CUDA to reduce VRAM usage.
        compute_dtype = torch.float16 if img.is_cuda else img.dtype
        img = img.to(dtype=compute_dtype)
        depth = depth.to(device=img.device, dtype=compute_dtype)
        if torch.is_tensor(foc_dist):
            foc_dist = foc_dist.to(device=img.device, dtype=compute_dtype)
        ...
        if len(img.shape) == 3:
            ...
            z = self.depth2z(depth)
            x, y = torch.meshgrid(
                torch.linspace(-1, 1, W, device=img.device, dtype=compute_dtype),
                torch.linspace(1, -1, H, device=img.device, dtype=compute_dtype),
                indexing='xy'
            )
            ...
            render_lr = local_psf_render(img_degamma, psf, self.kernel_size)
            render = torch.cat(render_lr, dim=1)
