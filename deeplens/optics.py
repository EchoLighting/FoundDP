...
class Lensgroup(DeepObj):
    ...
    """
    def __init__(self, filename=None, sensor_res=(1024, 1024), use_roc=False, post_computation=True, same_sampling=False, device=DEVICE):
        ...
        self.to(device)
        # for same_sampling
        self.same_sampling = same_sampling  #False#True
        self.o2 = None
        # for same_sampling end
    ...
    def prepare_sensor(self, sensor_res=[512, 512], sensor_size=[24.,36.], sensor_directly=True):
        """ Create sensor. 
        ...
        # self.sensor_size = self.sensor_size if sensor_directly is False else[24,36]
    ...
    def sample_from_points(self, o=[[0, 0, -10000]], spp=256, wvln=DEFAULT_WAVE, shrink_pupil=False, normalized=False):
        ...
        ###########################
        # for same_sampling
        cheif_ray_processing = True if shrink_pupil==True else False
        if self.same_sampling:
            if self.o2 is None and cheif_ray_processing == False:
                self.o2 = o2
            if cheif_ray_processing == False:
                o2 = self.o2
        ###########################
        d = o2.unsqueeze(1) - o
    ...
    def set_same_sampling(self,):
        self.same_sampling = True
        self.o2 = None
    
    def set_random_sampling(self,):
        self.same_sampling = False
        self.o2 = None
    ...
    # ====================================================================================
    @torch.no_grad()
    def render_single_img(self, img_org, depth=DEPTH, spp=64, unwarp=False, save_name=None, return_tensor=False, noise=0, method='psf'):
    ...
    def point_source_grid(self, depth, grid=9, normalized=True, quater=False, center=False):
        ...
        if quater:
            ...
            point_source = point_source[0:bound_i, 0:bound_i, :]
        if not normalized:
    ...
    def psf_diff(self, points, wvln=DEFAULT_WAVE, ks=31, spp=GEO_SPP, center=True, param_list=None):
        """ Single wvln incoherent PSF calculation.
        ...
        if center:
            ...
            psf = forward_integral(ray, ps=self.pixel_size, ks=ks, pointc_ref=pointc_chief_ray, param_list=param_list)
        else:
            ...
            psf = forward_integral(ray, ps=self.pixel_size, ks=ks, pointc_ref=pointc_ideal, param_list=param_list)
        
        # # Normalize to 1
        light_nums = ray.d.shape[0]
        lum = psf.sum(-1).sum(-1) / light_nums
        # print(lum)

        # my way
        B,W,H=psf.shape
        values, indices = torch.max(psf.view(B,-1),dim=-1)
        values = values.unsqueeze(-1).unsqueeze(-1)
        psf = psf / (values+1e-6)

        # # # singer way
        # psf = psf / psf.sum(-1).sum(-1).unsqueeze(-1).unsqueeze(-1)
        # psf = psf / psf.sum(-1).sum(-1).unsqueeze(-1).unsqueeze(-1) * lum.unsqueeze(-1).unsqueeze(-1)
    ...
    def psf_rgb(self, points, ks=31, spp=GEO_SPP, center=True, param_list=None):
        ...
        for wvln in WAVE_RGB:
            psfs.append(self.psf_diff(points=points, wvln=wvln, ks=ks, spp=spp, center=center, param_list=param_list))
    ...
    def psf_map(self, depth=DEPTH, grid=7, ks=51, spp=GEO_SPP, center=True):
        ...
        if quater:
            grid = grid // 2 if grid % 2 == 0 else grid // 2 + 1
        points = points.reshape(-1, 3)
        psfs = self.psf_rgb(points=points, ks=ks, center=center, spp=spp) # shape [grid**2, 3, ks, ks]
        psf = psfs.reshape(grid**2,3,ks,ks)
        psf_l_map = make_grid(psf, nrow=grid, padding=0, pad_value=0.0)
        return psf_l_map
    ...
    # ====================================================================================
    @torch.no_grad()
    def analysis(self, save_name='./test', ks=None, render=False, multi_plot=False, plot_invalid=True, zmx_format=False, depth=DEPTH, render_unwarp=False, lens_title=None):
        ...
        self.draw_psf_map(save_name=save_name, ks=ks, depth=depth)
    ...
    import numpy as np  
    def corr_analysis(self, save_name='./test', ks=None, render=False, multi_plot=False, plot_invalid=True, zmx_format=False, depth=DEPTH, render_unwarp=False, lens_title=None, param_list=None, diagonal_points=False):
        """ Analyze the optical lens.
        # draw from https://blog.51cto.com/u_14693305/5710201
        """
        # Draw spot diagram and PSF map
        # center = torch.tensor([[0.8,-0.8,depth]],dtype=torch.float)
        v = 0.0
        if param_list is not None:
            lamda,h,f,leak,radius,v,direct=param_list
        center = torch.tensor([[v,0,depth]],dtype=torch.float)
        if diagonal_points ==True:
            center = torch.tensor([[v,v,depth]],dtype=torch.float)
        psf = self.psf_rgb(points=center, ks=ks, center=True, spp=GEO_SPP*50, param_list=param_list) # shape [grid**2, 3, ks, ks]
        psf = (psf).reshape(3,ks,ks)

        gray = True
        if gray:
            psf_map = psf.permute(1, 2, 0)
            gravity = torch.tensor([0.299,0.587,0.114]).to(self.device).unsqueeze(-1)
            psf_map =torch.matmul(psf_map,gravity)[...,0]
            # psf_map[psf_map>1e-6]=psf_map[psf_map>1e-6]+0.4
        ...
        y = np.linspace(ks//2, -ks//2, ks)  
        X, Y = np.meshgrid(x, y)  
        
        # 定义Z作为X和Y的函数（例如，Z = X^2 + Y^2）  
        Z = psf_map.cpu().numpy()
        plt.imshow(Z)
        save_name =  f'./psf{-depth}mm_left.png' if save_name is None else f'{save_name}_psf{-depth}mm_corr.png'
        ...
        x = np.linspace(-(ks//2), ks//2, ks)  
        y_c = (Z[ks//2]*4+Z[ks//2-1]*2+Z[ks//2+1]*2+Z[ks//2+2]+Z[ks//2-2])/10
        y_c_r = y_c[::-1]
        yc_up = y_c+y_c_r*0.0
        ycr_up = y_c_r+y_c*0.0

        print(y_c.sum())
        plt.figure()
        plt.plot(x, y_c, label='l_line', color='blue')
        plt.plot(x, y_c_r, label='r_line', color='red')
        plt.plot(x, yc_up, label='lu_line', color='black')
        plt.plot(x, ycr_up, label='lru_line', color='green')
    ...
    @torch.no_grad()
    def draw_psf_map(self, grid=9, depth=DEPTH, ks=51, log_scale=False, quater=False, save_name=None):
        """ Draw RGB PSF map at a certain depth. Will draw M x M PSFs, each of size ks x ks.
        """
        # Calculate PSF map
        psf_map = self.psf_map(depth=depth, grid=grid, ks=ks, spp=GEO_SPP*30, center=True)
        
        # Convert to gray, with cmap='gray'
        gray = False
        if gray:
            psf_map = psf_map.permute(1, 2, 0)
            gravity = torch.tensor([0.299,0.587,0.114]).cuda().unsqueeze(-1)
            psf_map =torch.matmul(psf_map,gravity)
            psf_map = psf_map.permute(2, 0, 1)
        ...
        draw_pixel = False
        if draw_pixel:
            grid = grid//2+1 if quater else grid
            plt.figure(figsize=(grid*ks, grid*ks),dpi=1)
        else:
            plt.figure(figsize=(3000, 3000),dpi=1)
        ...
        # save_name = f'./psf{-depth}mm_left.png' # if save_name is None else f'{save_name}_psf{-depth}mm_{angle}.png'
        save_name =  f'./psf{-depth}mm_left.png' if save_name is None else f'{save_name}_psf{-depth}mm_left.png'
        plt.savefig(save_name)
        plt.close()
