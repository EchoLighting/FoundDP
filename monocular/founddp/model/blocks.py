...
class FeatureFusionDepthBlock(nn.Module):
    ...
    def __init__(self, features, activation, deconv=False, bn=False, expand=False, align_corners=True, size=None):
        ...
        self.gate_conv = nn.Sequential(
            nn.Conv2d(1, features//4, kernel_size=3, padding=1, bias=True),
            nn.ReLU(inplace=True),
            nn.Conv2d(features//4, 1, kernel_size=3, padding=1, bias=True),
            nn.Sigmoid()   
        )
        self.skip_add = nn.quantized.FloatFunctional()
        self.size = size
    ...
    def forward(self, *xs, prompt_depth=None, size=None):
        ...
        if prompt_depth is not None:
            ...
            if prompt_depth.shape[1] == 2:
                pd = prompt_depth[:, 0:1, :, :]
                conf = prompt_depth[:, 1:2, :, :]
            else:
                pd = prompt_depth
                conf = torch.ones_like(pd)
            res = self.resConfUnit_depth(pd)
            gate_from_conf = self.gate_conv(conf)
            output = self.skip_add.add(output, res * gate_from_conf)
