...
from monocular.founddp.model.dpt import DPTHead
from monocular.founddp.model.config import model_configs
from monocular.founddp.utils.logger import Log
import os
...
class FoundDP(nn.Module):
    ...
    def __init__(self,
                 encoder='vitl',
                 ckpt_path='checkpoints/founddp_vitl.ckpt'):
        super().__init__()
        model_config = model_configs[encoder]

        self.encoder = encoder
        self.model_config = model_config
        module_path = Path(__file__) # From anywhere else: module_path = Path(inspect.getfile(FoundDP))
        package_base_dir = str(Path(*module_path.parts[:-2])) # extract path to FoundDP module, then resolve to repo base dir for dinov2 load
    ...
    def load_checkpoint(self, ckpt_path):
        if os.path.exists(ckpt_path):
            ...
            for k,v in checkpoint['state_dict'].items():
                k1 = k[9:]
                if k1 in net_dict and net_dict[k1].shape == checkpoint['state_dict'][k].shape:
                    update_dict[k1]=v
    ...
    def forward(self, x, prompt_depth=None):
        ...
        if prompt_depth is not None:
            ...
            prompt_depth = F.interpolate(prompt_depth, (518,784), mode='bilinear')
        x = F.interpolate(x, (518,784), mode='bilinear') 
        h, w = x.shape[-2:]
        with torch.no_grad():
        ...
        depth = F.interpolate(depth, (512,768), mode='bilinear')
        return depth
    
    def VIT_refinement(self, x):
        x = F.interpolate(x, (518,784), mode='bilinear') 
        features = self.pretrained.get_intermediate_layers(
            (x - self._mean) / self._std, self.model_config['layer_idxs'],
            return_class_token=True)
